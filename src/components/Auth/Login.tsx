...
export default function Login() {
  ...
  const { login, signInWithGoogle, signUpWithGoogle } = useAuth();
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      toast({
        title: `Google ${mode === 'login' ? 'Sign In' : 'Sign Up'} Failed`,
        description: error.message || `An error occurred during Google ${mode === 'login' ? 'sign in' : 'sign up'}`,
        variant: "destructive",
      });
      setRedirectInProgress(false);
    } finally {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex flex-col space-y-4">
            ...
            >
              ...
              <span>{redirectInProgress ? `Redirecting to Google...` : `Sign ${mode === 'login' ? 'in' : 'up'} with Google`}</span>
            </Button>
